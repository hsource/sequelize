...
Create a model by using an interface to extend the base Model, and then pass in the Model type as a Generic argument to the `sequelize.define` function.
...
interface MyModel extends Model<MyModelAttributes>, MyModelAttributes {
  readonly id: number;
}
